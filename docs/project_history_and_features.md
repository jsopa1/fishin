# Project history and how it got here

These sections used to live in the top-level README. They are kept here in full, unedited, because
they are the record of *how* the app was built and what each phase added. Some describe earlier
screens (the V2/V3 spot page and the waterbody directory) that the V4 redesign has since replaced;
the current screens are in the [README](../README.md#the-five-screens) and
[`docs/diagrams/screen_map.svg`](diagrams/screen_map.svg).
...
Full plan and what shipped: [`docs/v2_fish_intelligence_platform_plan.md`](v2_fish_intelligence_platform_plan.md) · bait research: [`docs/v1_bait_technique_research_report.md`](v1_bait_technique_research_report.md) · redesign: [`docs/v2_ux_redesign_report.md`](v2_ux_redesign_report.md).

Full write-up, including real data sources that were investigated and deliberately *not* shipped (with the reasoning behind each): [`docs/v2_access_points_report.md`](v2_access_points_report.md).
...
- **Every phase was scoped, executed, and gated behind explicit review** before the next began — [`DECISIONS.md`](../DECISIONS.md) is the literal, unedited audit trail: 56 numbered decisions, each with its own rationale, including the ones that reversed course.
