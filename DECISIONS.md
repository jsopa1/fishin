...
**Addendum: spot maps on cards, and the range meter.** Ranked cards (Home's Recommended and Saved lists, and Explore's list) open with a small map of the spot, four OpenStreetMap tiles with a pin, because there is no photograph of any individual spot in the data and a fish photo there implied a promise about the water. (An earlier pass used species photos; the owner asked for the spot instead.) The tiles come straight from OpenStreetMap to the visitor's browser, name only the spot's map area, and are credited on Home and covered on the privacy page. Spot fish rows carry a thin meter of the documented range on a 30-90 F scale with today's temperature marked: sage for the feeding window, blue for the spawning range. The first version drew only the feeding window, so the 3,112 rows (of 22,199) where a fish is Active because it is inside its spawning range showed a marker far from any bar, and 42 more sat up to half a degree off a bar because the row prints a whole degree against decimal window edges. Both are fixed (the meter now draws both ranges and positions the marker from the unrounded temperature) and a test checks, across many spots, that every Active marker lies inside a drawn range and no Inactive marker lies inside its feeding window.
